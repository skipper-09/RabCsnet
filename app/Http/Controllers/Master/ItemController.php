...
class ItemController extends Controller
{
    ...
    {
        $dataType = Item::with(['typeItem', 'unit'])  // Eager load unit and typeItem
            ->orderByDesc('created_at')
            ->get();

        return DataTables::of($dataType)
            ->addIndexColumn()
            ->addColumn('typeItem_name', function ($item) {
                return $item->typeItem ? $item->typeItem->name : '-';  // Ensure correct access to typeItem
            })
